// コンテンツ一覧の定義情報（contents.json）を管理するコントローラー。
//
// 起動時にアプリ内サーバーから JSON を取得し、
// オフライン・サーバー未起動時はローカルキャッシュ→バンドルアセットの順にフォールバックする。
// サーバーの Date ヘッダーを「信頼できる時刻」として利用することで、
// 端末時計が狂っていても公開期間の判定を正確に行える。
import 'dart:convert';
